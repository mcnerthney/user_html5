...
As `ngZoute` is quite minimal, take a look at the two provided submodules
...
.controller( 'AppCtrl', function AppCtrl ( $scope, $location ) {
  $scope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams){
    if ( angular.isDefined( toState.data.pageTitle ) ) {
      $scope.pageTitle = toState.data.pageTitle + ' | ngZoute' ;
    }
  });
})
```
...
One of the design philosophies of `ngZoute` is that tests should exist
