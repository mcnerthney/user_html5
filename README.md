# [ngZoute](http://joshdmiller.github.com/ng-boilerplate) [![Build Status](https://api.travis-ci.org/ngbp/ngbp.png?branch=v0.3.2-release)](https://travis-ci.org/ngbp/ngbp)
...
`ngZoute` is designed to make life easy by providing a basic framework
...
ngZoute also comes prepackaged with the most popular design frameworks
...
The principal goal of `ngZoute` is to set projects up for long-term
success.  So `ngZoute` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `ngZoute`.

But `ngZoute` is not an example of an AngularJS app: this is a
...
getting `ngZoute` up and running. Though `ngZoute` is really simple
...
`ngZoute` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `ngZoute`, like Twitter Bootstrap
...
Technically, `ngZoute` is now ready to go.
...
`ngZoute`. What follows in this section is a quick introduction to the
...
`ngZoute` also includes [Live Reload](http://livereload.com/), so you no
...
default with `ngZoute`) you need to tell Live Reload to allow it. Go to
