# The Tools Used in `ngZoute`

## Introduction

`ngZoute` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ngZoute` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ngZoute` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ngZoute`, this project makes the assumption
...
While `ngZoute` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ngZoute` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `ngZoute`; Grunt is
...
to get started with `ngZoute`. But for those curious or looking to go a
...
Grunt is the engine behind `ngZoute`. It's the magic that makes it move.
...
`ngZoute` comes with a `bower.json` file that looks something like this:
...
in `ngZoute` by default in there to see what I mean.
